	...
	private class OdinAgentFailureDetectorTask extends TimerTask {
		...
		public void run() {
			...
			if ((System.currentTimeMillis() - agent.getLastHeard()) >= agentTimeout) {
				...
				for(Iterator<OdinClient> it = ((OdinAgent)agent).mpClientList.iterator(); it.hasNext(); )
				{
					OdinClient oc = it.next();
					Lvap mvap = oc.findMvapWithSpecAgent(agent);
					if(mvap != null)
					{
						ConcurrentHashMap<TCPFlowID, Lvap> map = oc.getMapOfFlow();
						for(Iterator<Map.Entry<TCPFlowID, Lvap>> it1 = map.entrySet().iterator(); it1.hasNext();)
						{
							Map.Entry<TCPFlowID, Lvap> entry = it1.next();
							TCPFlowID key = entry.getKey();
							Lvap val = entry.getValue();
							if(val == mvap)
							{
								it1.remove();
							}
				...
				//clear all the agent's client's mptcp data structure
