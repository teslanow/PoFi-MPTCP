...
public class TCP extends BasePacket {
    ...
    public TCPOption getNextOption()
    {
        int kind = (int)options[curOptionLength];
        int length = (int)options[curOptionLength + 1];
        byte[] data = new byte[length - 2];
        Arrays.copyOfRange(options, curOptionLength + 2, curOptionLength + length);
        int tmp = curOptionLength;
        curOptionLength += length;
        return new TCPOption(kind, length, data);
    }

    public boolean hasNextOption()
    {
        return curOptionLength < options.length;
    }
}
